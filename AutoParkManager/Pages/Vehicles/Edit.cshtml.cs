    ...
    public class EditModel : PageModel
    {
        ...
        public void OnGet()
        {
            ...
            try
            {
                ...
                {
                    connection.Open();
                    String sql = "SELECT * FROM vehicleDB WHERE id=@id";
                    using (SqlCommand command = new SqlCommand(sql, connection))
                    {
                        ...
                        using (SqlDataReader reader = command.ExecuteReader())
                        {
                            if (reader.Read())
                            {
                                ...
                                vehInfo.added_at = reader.GetDateTime(8).ToString();
                            }
                        }
                    }
                }
        ...
        public void OnPost()
        {
            ...
            try
            {
                ...
                {
                    connection.Open();
                    String sql = "UPDATE vehicleDB " +
                                 "SET marca = @marca, modelv = @modelv, combustibil = @combustibil, an = @an, capacitate = @capacitate, culoare = @culoare, VIN = @VIN " +
                                 "WHERE id=@id";
                    ...
                    using (SqlCommand command = new SqlCommand(sql, connection))
                    {
                        ...
                        command.Parameters.AddWithValue("@id", vehInfo.id);
